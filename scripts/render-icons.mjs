...
const CANDIDATES = [
  ...
  "chromium",
  "chromium-browser",
  path.join(os.homedir(), ".cache/ms-playwright/chromium_headless_shell-1208/chrome-linux/headless_shell"),
].filter(Boolean);
...
async function launch() {
  for (const bin of CANDIDATES) {
    const proc = spawn(bin, [
      ...
      "--hide-scrollbars", `--user-data-dir=${path.join(os.tmpdir(), "render-icons-profile")}`, "about:blank",
    ], { stdio: "ignore" });
    const failed = new Promise((r) => proc.on("error", () => r(true)));
    for (let i = 0; i < 40; i++) {
      if (await Promise.race([failed, sleep(250).then(() => false)])) break;
      try { await getJSON(`http://127.0.0.1:${PORT}/json/version`); return proc; } catch {}
    }
    proc.kill();
  }
...
class CDP {
  constructor(ws) { this.ws = ws; this.id = 0; this.p = new Map();
    ws.onmessage = (ev) => { const m = JSON.parse(ev.data); const h = this.p.get(m.id); if (h) { this.p.delete(m.id); m.error ? h.rej(new Error(m.error.message)) : h.res(m.result); } }; }
...
async function render(cdp, file, width, height, transparent) {
  ...
  await cdp.send("Page.navigate", { url: `file://${path.join(ROOT, "media-src", file)}` });
  for (let i = 0; i < 100; i++) { if ((await cdp.eval("document.readyState")) === "complete") break; await sleep(100); }
...
try {
  const target = (await getJSON(`http://127.0.0.1:${PORT}/json`)).find((t) => t.type === "page");
  ...
  await fs.writeFile(path.join(ROOT, "public/og-image.png"), await sharp(og).png({ compressionLevel: 9 }).toBuffer());

  const icon = await render(cdp, "icon.html", 512, 512, true);
  const at = (size) => sharp(icon).resize(size, size).png().toBuffer();
  await fs.writeFile(path.join(ROOT, "public/apple-touch-icon.png"), await at(180));
  await fs.writeFile(path.join(ROOT, "public/favicon-32.png"), await at(32));
  const entries = await Promise.all([16, 32, 48].map(async (size) => ({ size, data: await at(size) })));
  await fs.writeFile(path.join(ROOT, "public/favicon.ico"), toIco(entries));

  ws.close();
  console.log("wrote public/og-image.png, favicon.ico, favicon-32.png, apple-touch-icon.png");
} finally {
  chrome.kill();
}
